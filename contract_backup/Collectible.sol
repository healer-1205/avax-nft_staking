...
contract Collectible is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            if (itemId > 1) {
                runningMintBalance[itemId] = runningMintBalance[itemId - 1].add(
                    cost.mul(percentagePerMintToMinters).div(100).div(
                        itemId - 1
                    )
                );
            } else {
    ...
    function claimableRewards() external view returns (uint256) {
        ...
        for (uint256 i; i < senderTokenCount; i++) {
            ...
            claimableBalance = claimableBalance
                .add(runningMintBalance[supply])
                .sub(
                    runningMintBalance[lastClaimedMintIndexByTokenId[tokenId]]
                ); // for minters
            claimableBalance = claimableBalance
                .add(runningSaleBalanceForHolders[salesCount])
                .sub(
                    runningSaleBalanceForHolders[
                        lastClaimedSaleIndexForHoldersByTokenId[tokenId]
                    ]
                ); // for holders per sale
        }

        for (uint256 i; i < tokenIdsMintedByCreator[msg.sender].length; i++) {
            uint256 tokenId = tokenIdsMintedByCreator[msg.sender][i];
            claimableBalance = claimableBalance
                .add(runningSaleBalanceForCreator[salesCount])
                .sub(
                    runningSaleBalanceForCreator[
                        lastClaimedSaleIndexForCreatorsByTokenId[tokenId]
                    ]
                ); // for creators per sale
        }
    ...
    function setPercentagePerMintToMinters(uint256 _percentagePerMintToMinters)
        external
        onlyOwner
    {
    ...
    function setPercentageRoyalty(uint256 _percentageRoyalty)
        external
        onlyOwner
    {
    ...
    function setPercentageRoyaltyPerHolder(uint256 _percentageRoyaltyPerHolder)
        external
        onlyOwner
    {
    ...
    function setPercentageRoyaltyPerCreator(
        uint256 _percentageRoyaltyPerCreator
    ) external onlyOwner {
