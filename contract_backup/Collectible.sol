...
contract Collectible is ERC721Enumerable, Ownable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      if (itemId > 1) {
        runningMintBalance[itemId] = runningMintBalance[itemId - 1].add(
          cost.mul(percentagePerMintToMinters).div(100).div(itemId - 1)
        );
      } else {
  ...
  function claimableRewards() external view returns (uint256) {
    ...
    for (uint256 i; i < senderTokenCount; i++) {
      ...
      claimableBalance = claimableBalance.add(runningMintBalance[supply]).sub(
        runningMintBalance[lastClaimedMintIndexByTokenId[tokenId]]
      ); // for minters
      claimableBalance = claimableBalance.add(runningSaleBalanceForHolders[salesCount]).sub(
        runningSaleBalanceForHolders[lastClaimedSaleIndexForHoldersByTokenId[tokenId]]
    ...
    for (uint256 i; i < tokenIdsMintedByCreator[msg.sender].length; i++) {
      ...
      claimableBalance = claimableBalance.add(runningSaleBalanceForCreator[salesCount]).sub(
        runningSaleBalanceForCreator[lastClaimedSaleIndexForCreatorsByTokenId[tokenId]]
